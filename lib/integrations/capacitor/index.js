...
class Integration extends __1.BaseIntegration {
    ...
    async installCapacitorCore() {
        const [manager, ...managerArgs] = await npm_1.pkgManagerArgs(this.e.config.get('npmClient'), { command: 'install', pkg: '@capacitor/core@next' });
        await this.e.shell.run(manager, managerArgs, { cwd: this.root });
    }
    async installCapacitorCLI() {
        const [manager, ...managerArgs] = await npm_1.pkgManagerArgs(this.e.config.get('npmClient'), { command: 'install', pkg: '@capacitor/cli@next', saveDev: true });
        await this.e.shell.run(manager, managerArgs, { cwd: this.root });
    }
